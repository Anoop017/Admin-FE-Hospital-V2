      ...
      <table className="w-full text-left text-sm">
        <thead className="border-b bg-secondary/50">
          <tr>
            ...
            <th className="h-10 px-4 font-medium capitalize">Ward</th><th className="h-10 px-4 font-medium capitalize">bedNumber</th><th className="h-10 px-4 font-medium capitalize">status</th>
        ...
        <tbody>
          {items.map(item => (
            <tr key={item.id} className="border-b transition-colors hover:bg-muted/50">
              ...
              <td className="p-4 align-middle">{item.ward?.name || item.wardId?.toString()}</td><td className="p-4 align-middle">{item.bedNumber?.toString()}</td><td className="p-4 align-middle">{item.status?.toString()}</td>
